...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($act === 'create') {
    ...
    if ($name === '' || $slug === '') {
      ...
      $cek->execute();
      $exists = (bool) $cek->get_result()->fetch_row();
      $cek->close();
      ...
      if ($exists) {
        ...
        if (!$st->execute())
          $flash = 'Gagal menyimpan: ' . $st->error;
        $st->close();
        if (!$flash) {
          header('Location: quizzes.php?saved=1');
          exit;
        }
      }
    }
...
$alerts = [];
if (!empty($_GET['saved']))
  $alerts[] = ['type' => 'success', 'title' => 'Berhasil', 'msg' => 'Kuis tersimpan.'];
if (!empty($_GET['updated']))
  $alerts[] = ['type' => 'success', 'title' => 'Sukses', 'msg' => 'Data diperbarui.'];
if (!empty($_GET['deleted']))
  $alerts[] = ['type' => 'info', 'title' => 'Terhapus', 'msg' => 'Kuis berhasil dihapus.'];
if (!empty($flash))
  $alerts[] = ['type' => 'warn', 'title' => 'Perlu perhatian', 'msg' => $flash];
?>
    ...
    <tbody>
      <?php foreach ($rows as $c): ?>
        <tr>
          ...
          <td><?= $c['is_active'] ? '<span class="chip ok">Aktif</span>' : '<span class="chip muted">Nonaktif</span>' ?>
          </td>
          ...
          <td class="actions">
            <div class="grp">
              ...
              <!-- Hapus -->
              <form method="post" class="hide-on-edit" onsubmit="return confirm('Hapus kuis ini?')"
              ...
              <!-- Kelola Soal -->
              <a class="btn sm pink js-kelola-soal" href="quiz_manage.php?cat=<?= urlencode($c['slug']) ?>">Kelola
                Soal</a>
            </div>
          </td>
        </tr>
